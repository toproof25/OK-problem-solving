...
### ***의사 코드***
1. `N과 M을 입력받는다`
2. `동적 배열 array 크기 N만큼 할당한 후 0으로 초기화를 해준다`
3. `for (M번 반복하는 반복문)`
	1. `i번 ~ j번 인덱스 범위를 입력받고, 들어갈 숫자 number를 입력받는다`
	2. `for (i-1번 부터 j-1까지 반복)`
		1. `array[k]에 number를 할당한다`
4. `최종적으로 array에 각 값들을 출력한다`
5. `동적 메모리 할당을 제거한다`
