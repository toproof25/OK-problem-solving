...
### ***슈도 코드***
1. `cin >> number >> X - number`, X를 입력받는다
2. `int *array = new int[number]` - number크기만큼 동적 배열을 할당
3. `for (int i=0; i<number; i++)` - 반복 실행
	1. `if (X < array[i]) cout << array[i]`
4. `delete[] array`
